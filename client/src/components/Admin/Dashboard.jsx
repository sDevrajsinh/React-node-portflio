...
import { fetchAdminData, createProject, deleteProject, updateProject, requestAdminOTP, updateAdminCredentials, fetchUserProfile, verifyAdminOTP, uploadImage, API_BASE_URL, markMessageAsReadApi, fetchSettings, updateSettings } from '../../services/apiService';
...
const Dashboard = () => {
  ...
  const toggleSetting = async (key) => {
    ...
    try {
      ...
      setSettings(updated);
      await updateSettings(token, { [key]: newVal });
  ...
  const loadData = useCallback(async () => {
    const token = localStorage.getItem('adminToken');
    try {
      const [adminData, profileData, settingsData] = await Promise.all([
  ...
  const handleQuickUpdate = async () => {
    const token = localStorage.getItem('adminToken');
    try {
      // Direct update for non-sensitive profile info (bio, image, social links, resume)
      const res = await updateAdminCredentials(token, adminForm);
      if (res.success) {
      ...
      <main className="admin-main">
        ...
        <div className="admin-content">
          ...
          {activeTab === 'profile' && (
            ...
            {/* Quick Profile Management moved to Profile */}
            <div className="card-panel glass-panel" style={{ marginTop: '20px' }}>
              <div className="card-panel-header">
                <h3 className="card-panel-title">Portfolio Identity & Bio</h3>
                <button className="btn-primary btn-sm" onClick={handleQuickUpdate}>Save Bio Details</button>
              </div>
              <div className="form-grid">
                <div className="form-group full">
            ...
            <div className="settings-container">
              <div className="settings-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
                ...
                <div className="card-panel glass-panel" style={{ gridColumn: 'span 2' }}>
                  <div className="card-panel-header">
                    <h3 className="card-panel-title">SEO & Metadata Optimization</h3>
                    <button className="btn-primary btn-sm" onClick={handleSettingsSave}>Save SEO Data</button>
                  </div>
                  <div className="form-grid">
                    <div className="form-group full">
                    ...
                    <div className="form-group full">
                      <label>Meta Description (Shown in Google Search)</label>
                      <textarea 
                        className="form-input" 
                        rows="3" 
